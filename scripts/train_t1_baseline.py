"""Train the Task 1.2 baseline GRU encoder-decoder."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint_dir", type=Path, default=Path("checkpoints/t1_baseline"))
...
def main() -> None:
    ...
    args = parse_args()
    set_seed(42)
    logger = setup_logging(args.checkpoint_dir / "train.log", name="t1_baseline")
    device = get_device()
    ...
    config.update({"vocab_size": vocab_size, "device": str(device), "seed": 42, "cell": "GRU", "bidirectional": False})
